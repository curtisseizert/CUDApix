...
namespace S2dev{
  ...
  __device__ void exclusiveScan(uint32_t c, uint32_t * array, uint64_t & total);
  __device__ void exclusiveScan(uint32_t c, uint32_t * array, int64_t & total);
  __device__ void exclusiveScan(int64_t * array, uint64_t & total);
  __device__ int32_t computeMuPhi(uint32_t * s_count, uint32_t * s_sieve, int16_t * s_num, uint32_t p, S2data_64 * data, mRange & mr, nRange & nr); // want to -= this quantity
