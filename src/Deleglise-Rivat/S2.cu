...
#include "Deleglise-Rivat/S2.cuh"
#include "Deleglise-Rivat/deleglise-rivat.hpp"
#include "cudapix.hpp"
#include "pitable.cuh"

uint64_t deleglise_rivat64::S2()
{
  ...
  S2_a<<<blocks, threadsPerBlock>>>(x, pq.d_primes + pSoFar, d_sums, y, num_p, pq.len, d_pitable);
  cudaDeviceSynchronize();
...
__global__
void S2_a( uint64_t x, uint64_t * pq, uint64_t * sums,
...
__device__
inline uint64_t calculatePiChi(uint64_t quot, uint64_t y, uint32_t * d_pitable)
{
